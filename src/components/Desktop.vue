...
export default {
  ...
  mounted () {
    ...
    this.desktopItemPosition(this.originalDesktopItemList)
    window.onresize = () => {
      _this.calculatedQuantity()
      _this.desktopItemPosition(_this.originalDesktopItemList)
    }
  },
  methods: {
    /**
    ...
    desktopItemPosition (data) {
      ...
      desktopItemList.length = maxLength
      for (let i = 0; i < recordData.length && recordData.length <= maxLength; i++) {
        let item = recordData[i]
        if (item.position) {
          let position = item.position.split('-')
          let index = parseInt(position[0]) * this.desktopColNum + parseInt(position[1])
          if (index <= maxLength && !desktopItemList[index] && position[0] <= this.desktopRowNum && position[1] <= this.desktopColNum) {
            desktopItemList[index] = item
            recordData[i] = undefined
          }
        }
      }
      for (let col = 0; col < this.desktopColNum; col++) {
        for (let row = 0; row < this.desktopRowNum; row++) {
          let index = row * this.desktopColNum + col
          for (let i = 0; i < recordData.length && recordData.length <= maxLength; i++) {
            let item = recordData[i]
            if (!item || desktopItemList[index]) continue
            desktopItemList[index] = item
            recordData[i] = undefined
            break
          }
